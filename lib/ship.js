(function () {
  ...
  Ship.prototype.power = function (impulse, dir, ctx) {
    if (dir === "right") {
      ...
      if (floatOrientation < Math.PI) {
        floatOrientation += Math.PI/20;
      } else {
        floatOrientation = (Math.PI - (Math.abs(floatOrientation) - Math.PI)) * -1 + Math.PI/20;
      }

      this.orientation = floatOrientation.toFixed(20);

    } else if (dir === "down") {

      var floatVelDenom = parseFloat(this.velDenom.toString());

      if (floatVelDenom + 0.1 >= 5.0) {
        floatVelDenom = 5.0;
      } else {
        floatVelDenom += 0.1;
      }
