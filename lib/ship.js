(function () {
  ...
  Ship.prototype.draw = function (ctx) {
    ...
    // this.vel[0] = Math.abs((Math.PI/2 - Math.abs(this.orientation)) / (Math.PI/2));
    // this.vel[1] = Math.abs((Math.PI - Math.abs(this.orientation)) / (Math.PI/2));

    var denomX = Math.cos(Math.abs(this.orientation) % (Math.PI/2));
    denomX = denomX === 0 ? .05 : denomX;

    var denomY = Math.sin(Math.abs(this.orientation) % (Math.PI/2));
    denomY = denomY === 0 ? .05 : denomY;

    this.vel[0] = this.velFactor / denomX;
    this.vel[1] = this.velFactor / denomY;
